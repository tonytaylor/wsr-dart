...
$$.MatchImplementation = {"":
 ...
 groupCount$0: function() {
  var t1 = $.get$length(this._groups);
  if (typeof t1 !== 'number') return this.groupCount$0$bailout(1, t1);
  return t1 - 1;
 },
 groupCount$0$bailout: function(state, t1) {
  return $.sub(t1, 1);
 },
...
$$.main_anon = {"":
 ...
 $call$1: function(me) {
  $.regex = $.CTC3;
  for (var t1 = $.iterator($.allMatches($.regex, me.get$data())); t1.hasNext$0() === true; ) {
    var t2 = t1.next$0();
    $.print('global match: ' + $.S($.index(t2, 0)));
    $.print('source tag: ' + $.S($.index(t2, 1)));
    $.print('src attribute: ' + $.S($.index(t2, 2)));
    $.print('type attribute: ' + $.S($.index(t2, 3)));
    $.print('output contains ' + $.S(t2.groupCount$0()) + ' groups.');
  }
  $.regex.hasMatch$1(me.get$data()) !== true && $.print('regular expression has no match.');
  $.print($.StringBufferImpl$(['local: ', $.document().query$1('#output').get$innerHTML()]));
  $.print($.eq($.toString(me.get$data()), $.document().query$1('#output').get$innerHTML()));
  t1 = $.S(me.get$data());
  $.document().query$1('#output').set$innerHTML(t1);
 }
};
...
$.main = function() {
  var ws = $._WebSocketFactoryProvider_WebSocket('ws://localhost:15200/wsr-radio');
  $.add$1(ws.get$on().get$message(), new $.main_anon());
  $.add$1($.document().query$1('#serveClick').get$on().get$click(), new $.main_anon0(ws));
};
...
$.CTC3 = new Isolate.$isolateProperties.JSSyntaxRegExp(false, false, '(<source).+(src.+).+(type.+>)<');
